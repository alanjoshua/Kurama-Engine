...
//        head.setScale(10);
        ...
        var leftHand =
                "         [-1.7457e-02,  3.0444e-02, -1.6203e-02],\n" +
                        "         [-3.7675e-02,  4.3959e-02, -3.4716e-02],\n" +
                        "         [-4.9105e-02,  4.9542e-02, -5.9500e-02],\n" +
                        "         [-6.3489e-02,  5.7704e-02, -8.9622e-02],\n" +
                        "         [-5.4946e-02,  3.0550e-02, -6.2915e-02],\n" +
                        "         [-6.3320e-02,  4.6587e-02, -8.9723e-02],\n" +
                        "         [-6.0168e-02,  6.1337e-02, -1.0690e-01],\n" +
                        "         [-5.8606e-02,  7.9188e-02, -1.2337e-01],\n" +
                        "         [-4.2021e-02,  1.8516e-02, -7.4757e-02],\n" +
                        "         [-3.3017e-02,  3.6914e-02, -7.9357e-02],\n" +
                        "         [-2.1784e-02,  4.2597e-02, -6.7350e-02],\n" +
                        "         [-1.1487e-02,  4.1438e-02, -4.6440e-02],\n" +
                        "         [-2.1189e-02,  6.0973e-03, -7.6744e-02],\n" +
                        "         [-1.7557e-02,  2.4563e-02, -8.0639e-02],\n" +
                        "         [-8.7419e-03,  3.2240e-02, -6.5135e-02],\n" +
                        "         [-1.2329e-03,  3.2710e-02, -4.3419e-02],\n" +
                        "         [-2.3872e-03,  1.6551e-03, -7.3810e-02],\n" +
                        "         [-7.3582e-05,  1.5128e-02, -7.5248e-02],\n" +
                        "         [ 1.4194e-03,  2.1012e-02, -6.3136e-02],\n" +
                        "         [ 2.0630e-03,  2.2471e-02, -4.5250e-02], \n" +
                        "[0.0000e+00,  0.0000e+00,  0.0000e+00]";

        var rightHand =
                "         [ 0.0378,  0.0010, -0.0045],\n" +
                "         [ 0.0659, -0.0064, -0.0127],\n" +
                "         [ 0.0870, -0.0101, -0.0296],\n" +
                "         [ 0.1129, -0.0146, -0.0505],\n" +
                "         [ 0.0689, -0.0296, -0.0476],\n" +
                "         [ 0.0886, -0.0326, -0.0719],\n" +
                "         [ 0.1017, -0.0263, -0.0885],\n" +
                "         [ 0.1185, -0.0207, -0.1045],\n" +
                "         [ 0.0574, -0.0280, -0.0677],\n" +
                "         [ 0.0757, -0.0273, -0.0925],\n" +
                "         [ 0.0912, -0.0185, -0.1061],\n" +
                "         [ 0.1097, -0.0105, -0.1204],\n" +
                "         [ 0.0373, -0.0161, -0.0745],\n" +
                "         [ 0.0576, -0.0196, -0.0940],\n" +
                "         [ 0.0767, -0.0139, -0.1073],\n" +
                "         [ 0.0960, -0.0095, -0.1205],\n" +
                "         [ 0.0241, -0.0028, -0.0769],\n" +
                "         [ 0.0364, -0.0052, -0.0931],\n" +
                "         [ 0.0512, -0.0062, -0.1041],\n" +
                "         [ 0.0664, -0.0079, -0.1153],\n" +
                "[0.0000,  0.0000,  0.0000]";

        var leftHandModel = loadHand(leftHand);
        var rightHandModel = loadHand(rightHand);

        var offset = new Vector(0.2315f, -0.0052f,  0.0034f);
        var orientation = Quaternion.getQuaternionFromEuler(45, 0, 45);
        var scale = 100;

        leftHandModel.orientation = orientation;
        leftHandModel.setScale(scale);

        rightHandModel.orientation = orientation;
        rightHandModel.setScale(scale);
        rightHandModel.setPos(offset);


        renderer.addModel(rightHandModel);
        renderer.addModel(leftHandModel);

//        hand.setScale(new Vector(10,10,10));

        renderer.models.forEach(m -> m.tick(null, input, timeDelta, false));
        renderer.curFrameMeshletsDrawIndices = new ArrayList<>(IntStream.rangeClosed(0, 1).boxed().toList());
//        renderer.curFrameMeshletsDrawIndices.forEach(i -> renderer.meshlets.get(i).isRendered = true);
//        renderer.

        renderer.createMeshletsAndSyncGeoData( 21);
...
//                }
    ...
    public PointCloud loadHand(String data) {
        var threshold = 0.3f;
