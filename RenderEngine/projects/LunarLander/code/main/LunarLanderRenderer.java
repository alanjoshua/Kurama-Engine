...
public class LunarLanderRenderer extends VulkanRendererBase {
...
//            vkResetFences(device, drawFences.get(currentDisplayBufferIndex));
    ...
    public void cameraUpdatedEvent() {
        ...
        computeUBOIn.frustumPlanes = Arrays.asList(((LunarLanderGame)game).mainCamera.frustumIntersection.planes);
    }
...
//            }
    ...
    public void updateCameraGPUDataInMemory(int currentFrame) {
        ...
        bw.mapBuffer();
        GPUCameraData.memcpy(bw.buffer, gpuCameraData);

        if(multiViewNumLayers == 2) {
            bw.setPosition(1);
            GPUCameraData.memcpy(bw.buffer, gpuCameraData);
        }
...
//            uint32_t    firstInstance;
    ...
    public void initMultiViewSyncObjects() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            timelineCreateInfo.sType(VK_STRUCTURE_TYPE_SEMAPHORE_TYPE_CREATE_INFO);
            timelineCreateInfo.semaphoreType(VK_SEMAPHORE_TYPE_TIMELINE);
            timelineCreateInfo.initialValue(2);
            semaphoreInfo.pNext(timelineCreateInfo);
