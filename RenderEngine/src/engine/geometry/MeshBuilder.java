...
//		Construction of reflex and convex vertices lists
		for(int i = 0;i < verts.getSize();i++) {
			...
			if(engine.geometry.Utils.isVertexConvex(curr.previous.data,curr.data,curr.next.data,vertList)) {
...
//		Construction of ears list
		for(int i = 0;i < verts.getSize();i++) {
			Node<Vertex> curr = verts.peekNextNode();
			if(engine.geometry.Utils.isEar(curr.previous.data,curr.data,curr.next.data,reflex,vertList)) {
...
//			Remove one ear from top of list and create a new triangular Face object
				...
				if (reflex.isPresent(v0.data)) {
					if (engine.geometry.Utils.isVertexConvex(v0.previous.data, v0.data, currNode.data, vertList)) {
						reflex.searchAndRemoveNode(v0.data);
						convex.pushHead(v0.data);
						if (engine.geometry.Utils.isEar(v0.previous.data, v0.data, currNode.data, reflex, vertList)) {
				...
				if (reflex.isPresent(v2.data)) {
					if (Utils.isVertexConvex(currNode.data, v2.data, v2.next.data, vertList)) {
						...
						if (engine.geometry.Utils.isEar(currNode.data, v2.data, v2.next.data, reflex, vertList)) {
